...
export const InfiniteMovingCards = ({
  ...
  useEffect(() => {
    ...
    // Cleanup on unmount to prevent duplication issues
    return () => {
      if (scrollerRef.current) {
        scrollerRef.current.innerHTML = ''; // Clear existing children
      }
    };
      ...
      >
        {items.map((item, idx) => (
          <li
            ...
            style={{
              background: "linear-gradient(180deg, var(--sky-800), var(--sky-800)",
            }}
            key={item.name}
          >
            <blockquote>
